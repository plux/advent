...
%% For example:
%%
%% (()) and ()() both result in floor 0.  ((( and (()(()( both result
%% in floor 3.  ))((((( also results in floor 3.  ()) and ))( both
%% result in floor -1 (the first basement level).  ))) and )())())
%% both result in floor -3.  To what floor do the instructions take
%% Santa?
%%
%% --- Part Two ---
%%
...
%% causes him to enter the basement at character position 5.  What is
%% the position of the character that causes Santa to first enter the
%% basement?
...
solve(Input) ->
    lists:foldl(fun(C, {Floor, N}) ->
                        check_basement(Floor+calc(C), N),
                        {Floor+calc(C), N+1}
                end, {0, 1}, Input).

check_basement(-1, N) -> io:format("Hit basement @ ~p~n", [N]);
